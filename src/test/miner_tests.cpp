...
void MinerTestingSetup::TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vout[1].nValue = 100000000; // 1BLKH output
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    // //             pblock->hashMerkleRoot = BlockMerkleRoot(*pblock);
    // //             std::shared_ptr<const CBlock> shared_pblock = std::make_shared<const CBlock>(*pblock);
    // //             processBlock = m_node.chainman->ProcessNewBlock(chainparams, shared_pblock, true, nullptr);
    // //             if(processBlock) {
    // //                 std::cout << "nonce is " << shared_pblock->nNonce << std::endl;
    // //                 std::cout << "extranonce is " << j << std::endl;
    // //                 break;
    // //             }
    // //         }
    // //     }
