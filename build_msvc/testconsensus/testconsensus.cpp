...
// blinkhash includes.
#include <..\src\script\blinkhashconsensus.h>
...
int main()
{
    std::cout << "blinkhashconsensus version: " << blinkhashconsensus_version() << std::endl;
    ...
    auto op0Result = blinkhashconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, stream.data(), stream.size(), 0, blinkhashconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
